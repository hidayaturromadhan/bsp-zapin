...
<div class="container" style="max-width:1100px;">
  ...
  @php
    // group by bundle_id
    $groups = $versions->getCollection()->groupBy('bundle_id');
  @endphp
  ...
  <table border="1" cellpadding="8" cellspacing="0" style="width:100%; border-collapse:collapse; margin-top:10px;">
    ...
    <tbody>
      @forelse($groups as $bundleId => $rows)
        @php
        ...
        <tr>
          <td style="font-family:monospace; font-size:12px;">{{ $bundleId }}</td>

          <td>
            <div style="font-size:13px;">
              <div><b>ID:</b> {{ $idPayload['title'] ?? '-' }} <span style="color:#666;">/ {{ $idPayload['slug'] ?? '-' }}</span></div>
              <div><b>EN:</b> {{ $enPayload['title'] ?? '-' }} <span style="color:#666;">/ {{ $enPayload['slug'] ?? '-' }}</span></div>

              <div style="margin-top:6px; font-size:12px;">
                @if(!empty($idPayload['slug']))
                  <a target="_blank" href="{{ route('page.show', ['locale' => 'id', 'slug' => $idPayload['slug']]) }}">Preview ID</a>
                @endif
                @if(!empty($enPayload['slug']))
                  <span style="margin:0 8px;">|</span>
                  <a target="_blank" href="{{ route('page.show', ['locale' => 'en', 'slug' => $enPayload['slug']]) }}">Preview EN</a>
                @endif
              </div>
            </div>
          </td>
          ...
          <td>
            <form method="POST" action="{{ route('admin.pages.bundles.restore', ['page' => $page->id, 'bundle' => $bundleId]) }}">
              @csrf
              <button type="submit" onclick="return confirm('Restore bundle ini? (GLOBAL+ID+EN)')">Restore Bundle</button>
            </form>
          </td>
        </tr>
      @empty
        <tr><td colspan="4">Belum ada versi.</td></tr>
      @endforelse
    </tbody>
  </table>
