...
<form method="POST" action="{{ $action }}" enctype="multipart/form-data">
    ...
    <div class="a-card" style="margin-bottom:16px">
        <div class="a-card-head">
            <div>
                ...
                <div class="a-card-desc">Data utama program TJSL. Data akan disimpan sebagai draft sampai writer melakukan publish.</div>
            </div>
        </div>
    ...
    <div class="a-card" style="margin-bottom:16px">
        <div class="a-card-head">
            <div>
                ...
                <div class="a-card-desc">Cukup isi Bahasa Indonesia. Bahasa Inggris dibuat otomatis menggunakan DeepL.</div>
            </div>
        ...
        @if($trEn && ($trEn->title || $trEn->summary || $trEn->content))
            <div style="margin-top:18px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:14px">
                <div style="font-weight:900;color:#0f172a;margin-bottom:8px">Preview Terjemahan English Otomatis</div>
    ...
    <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;margin-bottom:16px">
        <a href="{{ route('writer.tjsl.index') }}" class="a-btn a-btn--secondary">Batal</a>

        @if($program->exists)
            <a href="{{ route('writer.tjsl.preview', $program) }}" target="_blank" class="a-btn a-btn--light">Preview</a>
        @endif

        <button type="submit" class="a-btn a-btn--primary">Simpan Draft</button>
    </div>
</form>
            ...
            @foreach($program->images as $image)
                <div style="border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;background:#fff">
                    ...
                    <div style="padding:10px">
                        ...
                        <form method="POST" action="{{ route('writer.tjsl.images.destroy', [$program, $image]) }}" onsubmit="return confirm('Hapus gambar ini?')">
                            @csrf
                            @method('DELETE')
